...
def simulation(cfg: DictConfig, mpc_configs: MultiPathChannelConfig, num_iter: int):
    ...
    optimizer = StochasticREPMSConjugateGradient(num_points_per_iter=cfg.num_points_per_iter,max_iterations=cfg.max_iterations, verbosity=cfg.verbose)
...
def main(cfg):
    ...
    data_path = os.path.join(file_path,'data', f'mpc_config_set_paths_{cfg.num_paths}_montecarlo_{cfg.num_monte_carlo}.npy')
    ...
    try:
        ...
        mpc_config_set = np.load(data_path, allow_pickle=True)
    # mpc_config_set = mpc_config_set[:2]
    results = Parallel(n_jobs=cfg.num_jobs)(delayed(simulation)(cfg,mpc_configs,n) for n, mpc_configs in enumerate(mpc_config_set))
