import torch
import numpy as np
from .ofdmchannel import OFDMConfig, MultiPathChannelConfig, OFDMBeamSpaceChannel
from .antenna_arrays import UniformLinearArray
from .utils import herm, db2lin
import opt_einsum as oe
from .prepare_sim import get_scale_matrix_J, get_prior_cov
...
def fim_c_xi(
    ...
    if isinstance(sig2_noise, (int, float)):
    ...
    term1 = oe.contract("nk, nkl, lt, ti, ij, nkj -> lj", 2 / sig2_noise, Lambda_nk.conj(), herm(T), C_x.conj(), T, Lambda_nk)
    term2 = herm(R) @ R
...
def obj_bfim(X, params_list, mpc_configs, cfg):
    ...
    for i in range(num_samples):
        newmpc = MultiPathChannelConfig(path_gains=new_b[i].numpy(),
                                    path_delays=new_tau[i].numpy(),
                                    doppler_shifts=new_fD[i].numpy(),
                                    aoas = new_aoas[i].numpy(),
                                    aods = new_aods[i].numpy())
        ofdmbschann = OFDMBeamSpaceChannel(mpc_configs=newmpc,ofdm_config=ofdm_configs, tx_array=tx_array, rx_array=rx_array)
        ...
        fimc_xi = fim_c_xi(mpc_configs=newmpc, ofdm_configs=ofdm_configs, tx_array=tx_array, rx_array=rx_array, w=w,channel_mat=Hf_tensor,sig2_noise=noise_sigma2)
    ...
    if not num_samples:
        ...
        fimc_xi = fim_c_xi(mpc_configs=mpc_configs, ofdm_configs=ofdm_configs, tx_array=tx_array, rx_array=rx_array, w=w,channel_mat=Hf_tensor,sig2_noise=noise_sigma2)
    ...
    return term1, term2, (term1 + cfg.multi_obj_factor * term2)