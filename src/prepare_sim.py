...
from .truncated_normal import TruncatedNormal, CONST_INV_SQRT_2
import numpy as np
...
def generate_mpc_config_set(cfg: DictConfig, path):
    ...
    mpc_config_set = np.array(mpc_config_set, dtype=object)
    np.save(path, mpc_config_set)


def get_sampler_list(cfg: DictConfig, mpc_configs):
    dist_br = MultivariateNormal(loc=from_numpy(mpc_configs.path_gains.real), scale_tril=torch.eye(cfg.num_paths) * cfg.path_gain.sigma * CONST_INV_SQRT_2)
    dist_bi = MultivariateNormal(loc=from_numpy(mpc_configs.path_gains.imag), scale_tril=torch.eye(cfg.num_paths) * cfg.path_gain.sigma * CONST_INV_SQRT_2)
    dist_tau = TruncatedNormal(loc=from_numpy(mpc_configs.path_delays), scale=cfg.delay.sigma,a = 0, b=torch.inf)
    dist_fD = MultivariateNormal(loc=from_numpy(mpc_configs.doppler_shifts), scale_tril=torch.eye(cfg.num_paths) * cfg.doppler.sigma)
    dist_aoas = TruncatedNormal(loc=from_numpy(mpc_configs.aoas), scale=cfg.aoa.sigma, a=-torch.pi/2, b = torch.pi/2)
    dist_aods = TruncatedNormal(loc=from_numpy(mpc_configs.aods), scale=cfg.aod.sigma, a=-torch.pi/2, b = torch.pi/2)
