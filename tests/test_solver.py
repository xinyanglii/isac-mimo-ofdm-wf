...
def test_manopt():
    ...
    assert len(uncons_result['objval_convergence']) == len(uncons_result['max_ineqval_convergence']) == len(uncons_result['gradient_norm_convergence']) == uncons_result['manopt_result'].iterations
    ...
    assert ineq_constraint(torch.from_numpy(cons_result['manopt_result'].point)).max() <= cons_optimizer.violation_tolerance
    assert cons_result['manopt_result'].iterations == len(cons_result['penalty_factors']) == len(cons_result['smoothing_factors'])
    ...
    stoch_determ_result = stoch_determ_optimizer.run(manifold,stoch_cost,param_sampler_list=[random_perturb_dist], ineq_constraint=ineq_constraint)
    assert ineq_constraint(torch.from_numpy(stoch_determ_result['manopt_result'].point)).max() <= stoch_determ_optimizer.violation_tolerance